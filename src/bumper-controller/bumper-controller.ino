/*
  $Id: bumper-controller-2023.ino 280 2023-11-16 15:13:00Z daisuke $
...
void setup() {
  ...
  LED.setPixelColor(0, LED.Color(200, 100, 50));
  ...
  CANID = CANID + ~ShiftData + 0x100; // 何故か最後に 0x100 を足さないと意図した CANID にならない
  MCANID = CANID + 0x100;
  ...
  delay(random(1000));
  data[0] = CANID;
  data[1] = BUMPER_TELEMETRY;
  CAN0.sendMsgBuf(MCANID, 0, 2, data);
}

// ----------------------------------------------------------------------
void loop() {
  static int h = random(360);
  static int b = 0;
  const int step = 5;
  static int ss = step;
